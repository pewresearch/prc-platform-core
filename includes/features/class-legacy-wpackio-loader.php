...
class WPackio {
	...
	public function __construct( $appName, $outputPath, $version, $type = 'plugin', $pluginPath = false, $themeType = 'regular' ) {
		...
		if ( ! in_array( $type, array( 'plugin', 'theme', 'block', 'quiz', 'interactive', 'feature' ), true ) ) {
		...
		} elseif ( 'block' === $this->type ) {
			// Change these to the correct paths for plugins/prc-block-library.
			$filepath = \trailingslashit( dirname( $this->pluginPath ) ) . $this->outputPath . '/';
			$url      = \trailingslashit( \plugins_url( $this->outputPath, $this->pluginPath ) );
		} elseif ( 'quiz' === $this->type ) {
			// Change these to the correct paths for plugins/prc-quiz.
			$filepath = \trailingslashit( dirname( $this->pluginPath ) ) . $this->outputPath . '/';
			$url      = \trailingslashit( \plugins_url( $this->outputPath, $this->pluginPath ) );
		} elseif ( 'interactive' === $this->type ) {
			// Change these to the correct paths for plugins/interactives.
			$filepath = \trailingslashit( dirname( $this->pluginPath ) ) . $this->outputPath . '/';
			$url      = \trailingslashit( \plugins_url( $this->outputPath, $this->pluginPath ) );
		} elseif ( 'feature' === $this->type ) {
			// Change these to the correct paths for plugins/features.
			$filepath = \trailingslashit( dirname( $this->pluginPath ) ) . $this->outputPath . '/';
			$url      = \trailingslashit( \plugins_url( $this->outputPath, $this->pluginPath ) );
		}
		...
		\add_action( 'wp_head', array( $this, 'printPublicPath' ), -1000 );
		// in case of admin, styles and scripts are printed before admin_head
		// so we use admin_print_scripts
		\add_action( 'admin_print_scripts', array( $this, 'printPublicPath' ), -1000 );
	}
	 ...
	 *
	 * The trypoint from `@wpackio/scripts/lib/entrypoint.js` automatically
	 ...
	 *
	 * TODO: This needs to be more aware, we need to clean this up so we dont have so much noise in <head>.
	 */
	public function printPublicPath() {
		$publicPath = $this->getUrl( '' );
		$handle = 'window.__wpackIo' . $this->sanitize_path( $this->appName . $this->outputPath );
		$jsCode     = $handle . '=\'' . esc_js( $publicPath ) . '\';';
	...
	public function getAssets( $name, $entryPoint, $config ) {
		...
		if ( $enqueue === false ) {
			throw new \LogicException( 'No assets found in the entry point. Make sure you are using @wpackio/scripts version 6+. ' . print_r( $manifest, true ) );
		}
	...
	public function getManifest( $dir ) {
		...
		if ( ! isset( $this->manifestCache[ $this->outputPath ] ) ) {
			$this->manifestCache[ $this->outputPath ] = array();
		}
