...
export default function RewritesPanel({ postType, postId, postSlug }) {
	...
	useEffect(() => {
		if (0 !== items.length) {
			...
			setMeta({ ...meta, _feature_rewrites: items });
		}
	}, [items]);
	...
	return (
		<BaseControl
			help={__(
				'You can add rewrites to your feature by using the `{myVar}` syntax to indicate URL parameters that you want to be accessible in the `prcURLVars` window namespace.'
			)}
		>
			<List
				lockVertically
