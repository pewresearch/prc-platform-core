 ...
 * Term Data Store
 * Forked from 10up.
 * Original author: John P. Bloch
 *
 * We've modified this slightly to enforce a stronger 1:1 relationship between term and post via id's stored in post and term meta respecitvely.
 */
	...
	function get_save_term_hook( $post_type, $taxonomy ) {
		...
		$closure = function ( $term_id ) use ( $post_type, $taxonomy ) {
			...
			if ( true === $create_post ) {
				...
				if ( null === $post ) {
					throw new General_Exception( 'Error getting post for term update:  Slug: ' . $term->slug . ' / Title: ' . $term->name );
				}
