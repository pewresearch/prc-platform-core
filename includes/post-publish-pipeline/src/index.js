...
addFilter('editor.preSavePost', 'editor', (edits) => {
	...
	if (isSiteEditor) {
		console.log('prc-platform.onSiteEdit', {
			edits,
			postId,
			postType,
			postStatus,
	...
	if (null === priorStatus && 'auto-draft' === postStatus) {
		console.log('prc-platform.onPostInit', {
			edits,
			postId,
			postType,
			postStatus,
	...
	if (
		('draft' === priorStatus || null === priorStatus) &&
		'draft' === postStatus
	) {
		console.log('prc-platform.onIncrementalSave', {
			edits,
			postId,
			postType,
			postStatus,
	...
	if ('draft' === priorStatus && 'publish' === postStatus) {
		console.log('prc-platform.onPublish', {
			edits,
			postId,
			postType,
			postStatus,
	...
	if (
		...
		console.log('prc-platform.onUpdate', {
			edits,
			postId,
			postType,
			postStatus,
	...
	if ('publish' === priorStatus && 'draft' === postStatus) {
		console.log('prc-platform.onUnpublish', {
			edits,
			postId,
			postType,
			postStatus,
