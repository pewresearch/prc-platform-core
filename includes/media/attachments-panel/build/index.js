...
const fetchImagesFromLegacyContent = (postId, postType = 'post') => {
  ...
  let legacyPostId = 0;
  let legacyBlogId = 0;
  const post = (0,_wordpress_data__WEBPACK_IMPORTED_MODULE_6__.select)('core').getEntityRecord('postType', postType, postId);
  if (post) {
  ...
  fetch(`https://prc-platform.vipdev.lndo.site/religion/wp-json/wp/v2/posts/${legacyPostId}?_fields=content`).then(response => response.json()).then(data => {
    const imagesArray = [];
    ...
    if (0 < imagesArray.length) {
      loadListOfImageUrlsIntoMediaLibrary(imagesArray, postId).then(success => console.log('success', success)).catch(error => console.error('error', error));
    }
  }).catch(error => console.error(error));
};
const loadListOfImageUrlsIntoMediaLibrary = (imagesArray, postId, whenDone = () => {}) => {
  console.log('imagesArray...', imagesArray.map(image => image.src));
