...
use TDS\get_relationship;

/**
 * Rewrites class for managing permalink structure and query variables.
 *
 * @package PRC\Platform
 * @since   1.0.0
 */
class Rewrites {
	...
	public function init( $loader = null ) {
		if ( null !== $loader ) {
			...
			$loader->add_action( 'template_redirect', $this, 'redirect_years_archive' );
		}
	}
	...
	public function redirect_years_archive() {
		// Check if we are on a year-based archive page
		if ( is_year() ) {
			// Get the year from the query vars
			$year = get_query_var( 'year' );
			if ( $year ) {
				// Check if this is also a post type archive page, and if so what post type.
				$post_type = is_post_type_archive() ? get_post_type() : null;
				// Build the new URL: /publications/?_years=2024
				if ( $post_type ) {
					$taxonomy = \TDS\get_relationship( $post_type );
					if ( $taxonomy ) {
